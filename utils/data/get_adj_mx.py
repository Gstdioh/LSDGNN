...
def get_adjacency_matrix(distance_file, sensor_ids, graph_type="direct", normalized_k=0.1):
    ...
    std = distances.std()
    adj_mx = np.exp(-np.square(dist_mx / std))  # 相当于加了自环，自己与自己连接是 1
...
def get_adjacency_matrix_nonormalized(distance_file, sensor_ids, graph_type="unDirect"):
    """
    构建邻接矩阵，双向，不用高斯核
    ...
    with open(distance_file, "r") as f_d:
        ...
        for item in reader:
            ...
            if row[0] not in sensor_id_to_ind or row[1] not in sensor_id_to_ind:
                continue
            A[sensor_id_to_ind[row[0]], sensor_id_to_ind[row[1]]] = 1
            dist_mx[sensor_id_to_ind[row[0]], sensor_id_to_ind[row[1]]] = row[2]

    # Make the adjacent matrix symmetric by taking the max.
    # 若是无向图，则构建对称矩阵
    if graph_type == "unDirect":
        A = np.maximum.reduce([A, A.T])
        dist_mx = np.maximum.reduce([dist_mx, dist_mx.T])
...
if __name__ == '__main__':
    ...
    #     graph_type="direct",
    #     normalized_k=0.1)
    sensor_ids, sensor_id_to_ind, adj_mx = get_adjacency_matrix_nonormalized(
        distance_file=r"D:\1_program\0_Traffic_Predition\My\demo_model\datasets\METR-LA\sensor_graph\distances_la_2012.csv",
        sensor_ids=r"D:\1_program\0_Traffic_Predition\My\demo_model\datasets\METR-LA\sensor_graph\graph_sensor_ids.txt",
        graph_type="unDirect")
